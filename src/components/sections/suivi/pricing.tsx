...
const reassuranceIcons = [Video, FileCheck2, ShieldCheck] as const;

export function SuiviPricing() {
  const { pricing } = suivi;
            ...
            <div className="grid lg:grid-cols-[minmax(0,1.18fr)_minmax(20rem,0.82fr)]">
              <div className="px-6 pb-7 pt-9 sm:px-9 sm:pb-8 sm:pt-11 lg:px-12 lg:pt-14">
                ...
                <ul className="mt-7 flex flex-wrap gap-2">
                  {pricing.reassurance.map((label, index) => {
                    const Icon = reassuranceIcons[index] ?? Video;
                    return (
                      <li
                        key={label}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-foreground">
                          {pricing.highlightTitle}
                        </p>
                        <p className="mt-1 text-xs leading-5 text-muted">
                          {pricing.highlightDescription}
                        </p>
                      </div>
                    </div>
                  </div>
